...
const MAX_TOTAL_SIZE = 15 * 1024 * 1024    // 15MB total per CAR
...
function CarAttachments({ carSubmissionId, canUpload = false, uploadedBy = null }) {
  ...
  async function loadFiles() {
    const { data } = await supabase
      ...
      .select('*')
      .eq('car_submission_id', carSubmissionId)
  ...
  async function handleUpload(selectedFiles) {
    ...
    if (files.length + selectedFiles.length > MAX_FILES) {
      setUploadError(`Maximum ${MAX_FILES} files allowed per CAR. You currently have ${files.length} file${files.length !== 1 ? 's' : ''} attached.`)
      setUploading(false)
      return
    }
    ...
    if (existingSize + newSize > MAX_TOTAL_SIZE) {
      setUploadError(`Total attachments cannot exceed 15MB per CAR. Current usage: ${formatFileSize(existingSize)}.`)
      setUploading(false)
      return
    }
    ...
    for (const file of selectedFiles) {
      ...
      const safeName = `${carSubmissionId}/${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`

      const { error: uploadErr } = await supabase.storage
        .from('car-attachments')
      ...
      await supabase.from('car_attachments').insert([{
        car_submission_id: carSubmissionId,
        file_name: file.name,
        file_path: safeName,
        file_size: file.size,
  ...
  function getPublicUrl(filePath) {
    return `${SUPABASE_URL}/storage/v1/object/public/car-attachments/${filePath}`
  }
        ...
        <div style={{ marginBottom: '16px' }}>
          {atLimit ? (
            <div style={{ padding: '14px 16px', backgroundColor: '#fef3c7', border: '1px solid #fde68a', borderRadius: '8px', fontSize: '13px', color: '#92400e', textAlign: 'center' }}>
              Maximum of {MAX_FILES} files reached for this CAR.
            </div>
          ) : (
            <div
            ...
            >
              ...
              <div style={{ fontSize: '12px', color: '#9ca3af' }}>
                Max {MAX_FILES} files · 5MB per file · 15MB total per CAR
              </div>
              <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '4px' }}>
                {files.length} of {MAX_FILES} files used · {formatFileSize(existingSize)} of 15MB used
              </div>
            </div>
          )}
          <input
